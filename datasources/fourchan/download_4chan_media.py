...
class FourchanSearchMediaDownloader(BasicProcessor):
    """
    Image downloader
    ...
    category = "Visual"
    # Default to images, but this is a generic media downloader that also handles .webm. See below for video preset.
    # Kept separate because of follow-up processors.
    title = "Download 4chan images"
    description = ("Download images from external archives and save them as a ZIP file. For the quickest results, "
                   "choose an archive that hosts the board used by this dataset. Some images may not be available on "
                   "the selected archive, so the results may be lower than the maximum requested."
    )
    extension = "zip"
    media_type = "image"
    ...
    def process(self):
        ...
        self.image_or_video = self.parameters.get("image_or_video", "image")

        # Setup state
        self.staging_area = self.dataset.get_staging_area()
        self.complete = False
        self.filenames = {}  # Maps URL -> Filenames
        self.api_url_id_map = defaultdict(list)  # Maps API URL -> post IDs
        self.url_id_map = defaultdict(list)  # Maps actual image URL -> post IDs (not the most efficient but hey)

        # Get Image URLs from source dataset through the chan archive APIs
        api_urls = self.get_api_urls()
        if not api_urls:
            self.dataset.finish_with_error("No archive API urls found in the dataset to get the media URLs.")
            return
        ...
        self.collect_media_urls(api_urls)

        if not self.filenames:
            self.dataset.finish_as_empty("No media URLs found after API search.")
            return
    ...
    def get_api_urls(self):
        ...
        for item in self.source_dataset.iterate_items(self):
            ...
            # Skip video / image based on preset
            if self.image_or_video == "image" and item.get("ext") in [".mp4", ".webm"]:
                continue
            elif self.image_or_video == "video" and item.get("ext") not in [".mp4", ".webm"]:
                continue
            ...
            search_urls.add(url)
            self.api_url_id_map[url].append(str(item['id']))
    ...
    def collect_media_urls(self, search_urls):
        """
        ...
        """
        self.dataset.update_status("Collecting media URLs from API")
        retry_counts = {}
        ...
        if self.archive_choice == "fourplebs":
            ...
            for search_url in search_urls:
                    ...
                    try:
                        ...
                        if resp.status_code != 200:
                            ...
                            if img_url:
                                ...
                                self.url_id_map[img_url] = self.api_url_id_map[search_url]
                                break  # Success
                ...
                self.dataset.update_status(f"Retrieved {len(self.filenames)}/{self.amount} media URLs")
                self.dataset.update_progress(len(self.filenames) / self.amount / 2)
                if len(self.filenames) >= self.amount > 0:
                    break

        # Proxied requests (Desuarchive)
        else:
    ...
    def download_media(self):
        """Downloads the files using the appropriate method per archive and media type."""
        ...
        self.dataset.update_status(f"Getting {len(targets):,} media files(s).")

        use_cloudscraper = (self.archive_choice == "fourplebs" and self.image_or_video == "video")

        if use_cloudscraper:
            # Cloudscraper for 4plebs videos; images work fine with proxies for some reason
            self.dataset.update_status("Using cloudscraper for 4plebs video downloads")
            scraper = cloudscraper.create_scraper(
                browser={"browser": "firefox", "platform": "windows", "mobile": False}
            )
            request_headers = {"User-Agent": self.UA, "Referer": "https://archive.4plebs.org/"}

            for image_url in targets:
                if self.interrupted:
                    self.flush_proxied_requests()
                ...
                try:
                    response = scraper.get(image_url, headers=request_headers, stream=True, timeout=20)
                    if response.status_code == 200:
                        with downloaded_path.open("wb") as outfile:
                    ...
                    failures.append(image_url)
                    downloaded_path.unlink(missing_ok=True)
                    self.dataset.update_status(f"Failed to download {image_url}: {e}")

                # Update metadata and progress
                metadata[image_url] = {
                    "filename": local_filename, "url": image_url, "success": success,
                    "from_dataset": self.source_dataset.key, "post_ids": self.url_id_map[image_url]
                }
                ...
                # Be polite to 4plebs to avoid bans
                time.sleep(1.5)

        else:
            self.dataset.update_status("Using proxied requests for downloads")

            # Dynamically set the referer for the proxied requests
            request_headers = {"User-Agent": self.UA}
            if self.archive_choice == "fourplebs":
                request_headers["Referer"] = "https://archive.4plebs.org/"
            elif self.archive_choice == "desuarchive":
                request_headers["Referer"] = "https://desuarchive.org/"
...
class FourchanSearchVideoDownloader(ProcessorPreset):
    """
    Video downloader
    Downloads top images and saves as zip archive
    """
    type = "preset-video-downloader-4chan"
    category = "Visual"
    # Default to images, but this is a generic media downloader that also handles .webm. See below for video preset.
    # Kept separate because of follow-up processors.
    title = "Download 4chan videos"
    description = ("Download .webm videos from external archives and save them as a ZIP file. For the quickest results,"
                   " choose an archive that hosts the board used by this dataset. Some videos may not be available on "
                   "the selected archive, so the results may be lower than the maximum requested."
    )
    extension = "zip"
    media_type = "video"
    ...
    def get_options(cls, parent_dataset=None, config=None):
        ...
        # Update the amount max and help from config
        max_number_media = int(config.get('video-downloader.max', 1000))
        if max_number_media != 0:
            options['amount']['help'] = f"No. of videos (max {max_number_media})"
            options['amount']['max'] = max_number_media
            options['amount']['min'] = 1
        else:
            options['amount']['help'] = "No. of videos"
            options['amount']["min"] = 0
            options['amount']["tooltip"] = "Set to 0 to download all videos"
    ...
    def get_processor_pipeline(self):
        """
        This queues the media downloader with media type set to video.
        """
        ...
        return [
            {
                ...
                "parameters": {
                    ...
                    "archive": self.parameters.get("archive", "desuarchive"),
                }
            }
        ]